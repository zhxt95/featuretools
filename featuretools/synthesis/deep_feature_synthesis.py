...
class DeepFeatureSynthesis(object):
    ...
    def _build_transform_features(self, all_features, entity, max_depth=0):
        ...
        for trans_prim in self.trans_primitives:
            ...
            matching_inputs = filt_trans_matching_inputs(matching_inputs) 
...
def filt_trans_matching_inputs(matching_inputs):
    filtered_matching_inputs = []
    for item in matching_inputs:
        ...
        if not all(isinstance(ftr,DirectFeature) for ftr in item):
            filtered_matching_inputs.append(item)
    return filtered_matching_inputs
